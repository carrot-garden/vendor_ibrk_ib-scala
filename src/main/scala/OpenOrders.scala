...
object OrderStatusData {
  def apply(orderId: Int, status: OrderStatus, filled: Int, remaining: Int, avgFillPrice: Double,
    permId: Int, parentId: Int, lastFillPrice: Double, clientId: Int, whyHeld: String) = new OrderStatusData(orderId, status, filled, remaining, avgFillPrice, permId, parentId, lastFillPrice, clientId, whyHeld)
}
...
trait OpenOrders extends EWrapper {
  ...
  def openOrder(orderId: Int, contract: Contract, order: Order, orderState: OrderState) {
    ...
    traceln(s"openOrder called: $orderId status: ${orderState.status}")
  }
