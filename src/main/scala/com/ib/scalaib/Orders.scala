...
class OrderDetailsExt(val ood: OpenOrderDetails) {
  def toDetailString: String = {
    ...
    s ++= s"Ticker: ${ood.contract.symbol}\n"
    s ++= s"Client ID: ${ood.order.clientId}\n"
    s ++= s"Perm ID: ${ood.order.permId}\n"
    s ++= s"Action: ${ood.order.action}\n"
    s ++= s"Quantity: ${ood.order.totalQuantity}\n"
    s ++= s"Order Type: ${ood.order.orderType}\n"
    s ++= s"Limit Price: ${ood.order.lmtPrice}\n"
    s ++= s"Aux Price: ${ood.order.auxPrice}\n"
    // OrderState fields
    s ++= s"Order Status: ${ood.orderState.status}\n" // Displays the order status.
...
class IbOrder(clientId: Int, orderId: Int, permId: Int, action: MktAction,
  ...
  this.permId(permId) // permanent ID that TWS assigns, for an initial order set to orderId
  this.action(action.toString)
  this.orderType(orderType.toString)
  this.totalQuantity(quantity)
  this.auxPrice(auxPrice)
  this.lmtPrice(limitPrice)
