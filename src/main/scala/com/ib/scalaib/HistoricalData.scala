...
trait HistoricalData extends IbPromise with Errors {
  ...
  type HistoricalDataFunc = ( /* reqId: */ Int, /* date: */ String, /* open: */ Double, /* high: */ Double, /* low: */ Double, /* close: */ Double, /* volume: */ Int, /* count: */ Int, /* WAP: */ Double, /* hasGaps: */ Boolean) => Unit
  ...
  def getHistoricalData(contract: Contract, endDateTime: String,
    ...
    traceln(s"getHistoricalData for: ${contract.symbol}")
        ...
        def histDataFunc(reqId: Int, date: String, open: Double, high: Double, low: Double,
          close: Double, volume: Int, count: Int, WAP: Double, hasGaps: Boolean) =
          {
            ...
            if (date.startsWith("finished")) {
              ...
              traceln(s"Hist Data for ${contract.symbol} completed")
            } else {
    ...
    val sub = obsHistData.subscribe(
      ...
      e => getAndRemovePromise(reqId) match {
        case Some(p) =>
          p.failure(e); traceln("Failure for hist data promise")
  ...
  private def getHistoricalData(reqId: Int, contract: Contract, endDateTime: String,
    ...
    // XXX
    //    client.reqHistoricalData(reqId, contract, endDateTime, duration, barSize, "TRADES", 1 /* use regular hours */ , 1)
  }
